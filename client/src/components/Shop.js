...
const getImageUrl = (imagePath) => {
    ...
    // Da Nginx den /uploads/-Pfad abfängt und die Bilder ausliefert,
    // geben wir den Pfad 1:1 zurück, ohne den Host-Namen (BACKEND_URL).
...
const Shop = () => {
    ...
    useEffect(() => {
        // fetch(API_URL) verwendet den korrigierten relativen Pfad
        fetch(API_URL) 
            .then(r => r.json())
            .then(data => {
